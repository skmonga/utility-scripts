    ...
    for line in f:
      ...
      if status == "EXIT":
        ...
        for window in dag_windows:
          ...
          window_end = window[1]
          window_time_taken_list = window[2]
          if entry_time >= window_start and entry_time <= window_end:
            window_time_taken_list.append(process_time - entry_time)
            break
          else:
            continue
...
#x_windows = [str(i) for i in range(1,11)]
x_windows = []
for window in dag_windows:
  label_window_start = (window[0] - violin_start_time)/1000
  label_window_end = (window[1] - violin_start_time)/1000
  x_windows.append(str(label_window_start))

x_windows.append(str((dag_windows[-1][1] - violin_start_time)/1000))
    
x_ticks_labels = x_windows
...
ax.set_xlim([0,2*len(x_windows)])

plt.xlabel('Time Window (sec)', fontsize = 15)
plt.ylabel('End-to-End Latency (ms)', fontsize = 15)
plt.title('Latency distribution for dataflows wrt time')
...
for i in range(1,len(x_windows)):
