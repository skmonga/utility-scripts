...
while dataflow_num <= total_dataflows:
  ...
  with open(end_file) as f:
    for line in f:
      ...
      status = splits[3].strip()
      if status == "EXIT":
        if dataflow_num%step_size == 0:
          window_num = dataflow_num/step_size
        else:
          window_num = dataflow_num/step_size + 1
        # window_num ranges from 1 to 10
        # check the corresponding dag_window
        matching_dag_window = dag_windows[window_num -1]
        matching_dag_window_start = matching_dag_window[0]
        matching_dag_window_end = matching_dag_window[1]
        if process_time >= matching_dag_window_start and process_time <= matching_dag_window_end:
          op_count_per_window[window_num] += 1
